...
let textGroup = document.createElementNS("http://www.w3.org/2000/svg", "g"); // dedicated text group
...
function generate() {
    ...
    state.charges.forEach(c => {
        ...
        // White dot for selected charge (not part of SVG download)
        if (c === selectedCharge) {
        ...
        if (state.showLabels) {
            ...
            const textStrokeEl = createSVG('text', {
                x: c.x + 0.1,
                y: c.y - 0.1,
                'font-size': state.textSize,
                fill: 'none',
                stroke: 'white',
                'stroke-width': state.textStroke
            });
            textStrokeEl.textContent = c.label;
            textStrokeEl.style.userSelect = 'none'; // prevent selection
            textGroup.appendChild(textStrokeEl);
            ...
            textFillEl.textContent = c.label;
            textFillEl.style.userSelect = 'none'; // prevent selection
            textGroup.appendChild(textFillEl);
        }
